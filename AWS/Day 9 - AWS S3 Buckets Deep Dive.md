...
![](../Attachments/Pasted%20image%2020241028012500.png)
![](../Attachments/Pasted%20image%2020241028012610.png)

2. I uploaded a simple .html file to later use it for a host simple site.(here changed the name to bryshten.com to connect it with my domain).

![](../Attachments/Pasted%20image%2020241028012845.png)

My IAM users should have S3FullAccess policy to see this bucket and the file. 
Let's restrict an access by setting bucket permissions.

3. Let's restrict all public access by setting bucket permissions.

Add new bucket policy
![](../Attachments/Pasted%20image%2020241028014909.png)

It uses json format. **Sid** is the name, **Principal** is for who is this policy against for(on our case it is for everyone). **Effect** is what should be done (Deny access to s3 in our case). **Action** is what actions in s3 should be denied, we added with Add action on the right.

![](../Attachments/Pasted%20image%2020241028015130.png)

**Resource** specifies the name of the Amazon resource that the policy applies to.

![](../Attachments/Pasted%20image%2020241028015330.png)

Add access to our account with the condition.
![](../Attachments/Pasted%20image%2020241028021906.png)

And add a line to the **Resource** section:
![](../Attachments/Pasted%20image%2020241028022237.png)


**Now let's make this bucket as static website. Go to properties and enable:
![](../Attachments/Pasted%20image%2020241028022513.png)

But the site is not accessible yet, access denied. We need to allow all public access and add:
![](../Attachments/Pasted%20image%2020241028022713.png)
**Note**: To connect this bucket host to Route53 domain the name of the bucket should be the same as domain name. 
